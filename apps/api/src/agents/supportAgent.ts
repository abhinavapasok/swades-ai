...
import { z } from 'zod'
import { searchFAQs, getConversationHistory, getUserInfo } from '../tools/supportTools.js'

const SUPPORT_SYSTEM_PROMPT = `You are a friendly and helpful customer support agent for an e-commerce company. Your role is to:
    ...
    return streamText({
      ...
      tools: {
        searchFAQs: tool({
          ...
          parameters: z.object({
            query: z.string().describe('The search query to find relevant FAQs'),
            category: z.string().optional().describe('Optional category filter: account, orders, returns, shipping, payment, support'),
          }),
        ...
        getConversationHistory: tool({
          description: 'Get previous messages from the current conversation for context',
          parameters: z.object({
