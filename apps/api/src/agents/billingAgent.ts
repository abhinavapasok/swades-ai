...
const BILLING_SYSTEM_PROMPT = `You are a billing specialist for an e-commerce company. Your role is to:
...
export const billingAgent = {
  ...
  description: 'Handles payment issues, refunds, invoices, and billing inquiries',
  capabilities: [
    ...
    return streamText({
      ...
      tools: {
        getInvoiceDetails: tool({
          description: 'Get complete details for a specific invoice including payment status and refund info',
          parameters: z.object({
        ...
        checkRefundStatus: tool({
          description: 'Check if a refund has been requested and its current status',
          parameters: z.object({
        ...
        getPaymentHistory: tool({
          description: 'Get a list of payment transactions for the current user',
          parameters: z.object({
            limit: z.number().default(10).describe('Number of recent payments to retrieve'),
          }),
          execute: async ({ limit }) => {
        ...
        requestRefund: tool({
          ...
          parameters: z.object({
            invoiceNumber: z.string().describe('The invoice number to request refund for'),
            amount: z.number().optional().describe('Partial refund amount (leave empty for full refund)'),
            reason: z.string().optional().describe('Reason for the refund request'),
          }),
